...
/*
Only use if going to use the zero_float_label
*/
void need_zero_float_label() {
  zero_float_flag = 1;
}

/*
Only use if going to use the one_float_label
*/
void need_one_float_label() {
  one_float_flag = 1;
}
    ...
    if (!idx) {
     ...
     res->instrs = append_seq(res->instrs, idx->instrs);
      ...
      if (!var_attr->is_addr) {
        ...
        append_seq(res->instrs, gen_instr(NULL, "addi", get_reg_name(idx->reg, TEMP_REG), get_reg_name(idx->reg, TEMP_REG), stack_name));
        append_seq(res->instrs, gen_instr(NULL, "add", get_reg_name(idx->reg, TEMP_REG), "$sp",  get_reg_name(idx->reg, TEMP_REG))); // maybe subu or little tricky math
  ...
  if (value) {
    ...
    if (is_func) {
      ...
      if (!idx) {
        ...
        append_seq(code, gen_instr(NULL, "sll", get_reg_name(idx->reg, TEMP_REG),get_reg_name(idx->reg, TEMP_REG), "2"));

        if (!var_attr->is_addr) {
          sprintf(stack_name, "%d", offset);
          append_seq(code, gen_instr(NULL, "addi", get_reg_name(idx->reg, TEMP_REG), get_reg_name(idx->reg, TEMP_REG), stack_name));
          append_seq(code, gen_instr(NULL, "add", get_reg_name(idx->reg, TEMP_REG), "$sp",  get_reg_name(idx->reg, TEMP_REG))); // maybe subu or little tricky math
          char* label = gen_paren_label(get_reg_name(idx->reg, TEMP_REG));
          ...
          sprintf(stack_name, "%d", offset);
          append_seq(code, gen_instr(NULL, "add", get_reg_name(array_addr, TEMP_REG),get_reg_name(array_addr, TEMP_REG), get_reg_name(idx->reg, TEMP_REG)));
          char* label = gen_paren_label(get_reg_name(array_addr, TEMP_REG));
          append_seq(code, gen_instr(NULL, var_type < FLOAT ? "sw" : "s.s", get_reg_name(value->reg, get_reg_type(var_type)), label, NULL));
          release_reg(array_addr, TEMP_REG);
        }
...
struct InstrSeq *local_save_seq(struct FunctionLocalSignature *locals) {
  ...
  size = 4;
  save = gen_instr(NULL, "sw", "$ra", "($sp)", NULL);
  ...
  return append_seq(gen_instr(NULL, "subu", "$sp", "$sp", offset), save);
}
...
struct InstrSeq *local_restore_seq(struct FunctionLocalSignature *locals) {
  ...
  size = 4;
  save = gen_instr(NULL, "lw", "$ra", "($sp)", NULL);
  ...
  return append_seq(save, gen_instr(NULL, "addi", "$sp", "$sp", offset));
}
...
void do_func_eval(struct FunctionSignature *func_info, struct FunctionParams *var_list, struct FunctionLocalSignature *local_list, struct InstrSeq *seq) {
  ...
  append_seq(func_attr->instrs, gen_instr(NULL, "jr", "$ra", NULL, NULL));//return things

...
int is_in_bounds(int val, int max) {
  return val >= 0 && val < max;
}

int get_avail_reg(enum register_type type) {
  int reg;

  if (type == TEMP_REG) {
    reg = avail_temp_reg();
    printf("REGISTER GIVEN: TEMP%d\n", reg);
  } else if (type == FLOAT_REG) {
    reg = avail_float_reg();
    printf("REGISTER GIVEN: FLOAT %d\n", reg);
  }

  return reg;
}

char *get_reg_name(int reg_num, enum register_type type) {
  if (type == TEMP_REG) {
    if (is_in_bounds(reg_num, MAX_TEMP_REG)) {
      return temp_registers[reg_num].name;
    }
  } else if (type == FLOAT_REG) {
    if (is_in_bounds(reg_num, MAX_FLOAT_REG)) {
...
int is_avail_reg(int reg_num, enum register_type reg_type) {
  if (reg_type == FLOAT_REG && is_in_bounds(reg_num, MAX_FLOAT_REG) && float_registers[reg_num].free) {
    return 1;
  } else if (reg_type == TEMP_REG && is_in_bounds(reg_num, MAX_TEMP_REG) && temp_registers[reg_num].free) {
    return 1;
  }
...
void release_reg(int reg_num, enum register_type type)
{
  printf("FREEING REG: %d\t, type: %d\n", reg_num, type);
  if (type == TEMP_REG) {
    if (is_in_bounds(reg_num, MAX_TEMP_REG)) {
